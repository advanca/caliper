...
module.exports.deploy = async function (networkConfig, account, privateKey, contractPath) {
    ...
    let contractDir = path.dirname(contractPath); 
    ...
    function findImports(pathDeclared) {
    ...
    let outputJson = solc.compileStandard(JSON.stringify(input), findImports);
    ...
    let contractBin = "";
    for (var name in output.contracts[contractPath]) {
